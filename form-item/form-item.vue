<template>
  <div class="lm-flex lm-flex-items-c lm-form-row" :class="cellClass">
    ...
    <p v-if="getInlineMessage">{{ validateMessage }}</p>
  </div>
</template>
<script>
import { dispatchMixin } from '../mixins.js'
import WxValidate from '../WxValidate.js'
import { isObject, isFunction, isArray, isString, getPropByPath, addEventHandle, getByName, cleanStyle, getParentProp } from '../utils.js'
import addMethodMap from '../validate.js'
export default {
  name: 'lm-form-item',
  ...
  mounted () {
    ...
    if (this.validate && this.rules && this.prop) {
      this.dispatch('lm-form', 'lm.form.addMethod', {
        'prop': this.prop,
        'rules': this.getRules(),
